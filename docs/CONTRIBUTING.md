...
- 道渊工坊属于可选第三方能力，只能通过 `src/features/image-library/workshop/` 访问；接口缺失、超时、报错或 Schema 无效时必须保留主图片库并继续渲染。
- 工坊图片只能在图片库领域层与主库合并，组件和 Pinia Store 不得直接读取 `window.parent.DaoyuanWorkshopAPI`。
