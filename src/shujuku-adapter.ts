// Shujuku Adapter
//
// This file is compiled as a standalone IIFE and injected at head-prepend so the
// database facade exists before Vue starts. Third-party table cells are
// intentionally modeled as explicit `any` values at this boundary; the typed
// application bridge normalizes them before stores consume them.
type AdapterRecord = Record<string, any>;
type AdapterRow = any[];

interface AdapterSheet {
  name: string;
  content: AdapterRow[];
...
interface AdapterApi {
  exportTableAsJson(): Record<string, AdapterSheet>;
  updateRow(tableName: string, rowIndex: number, values: AdapterRecord): Promise<unknown> | unknown;
  deleteRow(tableName: string, rowIndex: number): Promise<unknown> | unknown;
  insertRow(tableName: string, values: AdapterRecord): Promise<number> | number;
  updateCell(tableName: string, rowIndex: number, columnName: string, value: unknown): Promise<unknown> | unknown;
}

interface AdapterVariables extends AdapterRecord {
  stat_data: AdapterRecord;
}

type AdapterVariablesReader = () => AdapterVariables;

interface AdapterRoot {
  AutoCardUpdaterAPI?: AdapterApi;
  getAllVariables?: AdapterVariablesReader;
...
interface AdapterJadeContact extends AdapterLocatedRow {
  history: AdapterRecord;
}
...
(function installDaoyuanStatusDb() {
  ...
  const roots: AdapterRoot[] = [];
  const subscriptionStops = new Set<() => void>();
  let localVariablesProxy: AdapterVariablesReader | null = null;
  function addRoot(root: Window | null): void {
    try {
      const adapterRoot = root as unknown as AdapterRoot;
      if (root && !roots.includes(adapterRoot)) roots.push(adapterRoot);
    } catch (error) {}
  }
  addRoot(window);
  ...
  function getApi(): AdapterApi | null {
    for (const root of roots) {
      try {
  ...
  function getVariablesReader(): AdapterVariablesReader | null {
    for (const root of roots) {
      try {
  ...
  function readVariables(): AdapterVariables {
  ...
  function exportTables(): Record<string, AdapterSheet> {
    const api = getApi();
    if (!api || typeof api.exportTableAsJson !== 'function') return {};
    return api.exportTableAsJson() || {};
  }

  function getSheet(tableName: string): AdapterSheet | null {
  ...
  function locateRow(tableName: string, keyColumn: string, keyValue: unknown): AdapterLocatedRow | null {
  ...
  function resolvePath(path: readonly (string | number)[]): AdapterPathTarget | null {
  ...
  function filterWritableData(headers: AdapterRow, keyColumn: string, values: AdapterRecord): AdapterRecord {
    const allowed = new Set((headers || []).slice(1));
    allowed.delete(keyColumn);
    const result: AdapterRecord = {};
  ...
  async function update(path: readonly (string | number)[], values: AdapterRecord): Promise<boolean> {
    const api = getApi();
  ...
  async function remove(path: readonly (string | number)[]): Promise<boolean> {
    const api = getApi();
    const target = resolvePath(path);
  ...
  function parseHistory(value: unknown): AdapterRecord {
    let parsed: any = value;
    if (typeof parsed === 'string') {
      try { parsed = parsed ? JSON.parse(parsed) : {}; } catch (error) { parsed = {}; }
    }
    if (Array.isArray(parsed)) {
      const normalized: AdapterRecord = {};
  ...
  function readJadeContact(name: string): AdapterJadeContact | null {
  ...
  async function writeJadeHistory(name: string, history: AdapterRecord): Promise<boolean> {
    const api = getApi();
    if (!api) return false;
  ...
  async function appendJadeMessage(name: string, sender: string, content: string): Promise<{ success: boolean; history: AdapterRecord; messageId: string }> {
    const contact = readJadeContact(name);
  ...
  async function deleteJadeMessage(name: string, messageId: string): Promise<AdapterRecord> {
    const contact = readJadeContact(name);
    if (!contact) return {};
  ...
  async function ready(timeoutMs = 15000): Promise<boolean> {
  ...
  function subscribe(callback: () => void): { stop(): void } {
    ...
    const stop = () => {
      ...
      subscriptionStops.delete(stop);
    };
    subscriptionStops.add(stop);
    return { stop };
  }

  try {
    window.addEventListener('pagehide', () => {
      Array.from(subscriptionStops).forEach(stop => stop());
    }, { once: true });
  } catch (error) {}
  ...
  window.DaoyuanStatusDb = {
    ...
    ready,
    subscribe,
  } as unknown as NonNullable<Window["DaoyuanStatusDb"]>;
})();

function _dy_findApi(): AdapterApi | null {
  var wins: AdapterRoot[] = [window as unknown as AdapterRoot];
  try { if (window.parent && window.parent !== window) wins.push(window.parent as unknown as AdapterRoot); } catch(e) {}
  try { if (window.top && window.top !== window) wins.push(window.top as unknown as AdapterRoot); } catch(e) {}
  for (var i = 0; i < wins.length; i++) {
    const root = wins[i];
    try { if (root?.AutoCardUpdaterAPI) return root.AutoCardUpdaterAPI; } catch(e) {}
  }
  return null;
}

function getSheetByName(tableName: string): AdapterSheet | null {
  var api = _dy_findApi(); if (!api) return null;
  var all = api.exportTableAsJson();
  for (var key in all) {
    const candidate = all[key];
    if (key.indexOf('sheet_') === 0 && candidate?.name === tableName) return candidate;
  }
  for (var key in all) {
    const candidate = all[key];
    if (key.indexOf('sheet_') !== 0 || !candidate) continue;
    var ddl = candidate.sourceData?.ddl || '';
    var m = ddl.match(/CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(\w+)/i);
    if (m && m[1] === tableName) return candidate;
  }
  return null;
}

function getCellByHeader(tableName: string, rowIndex: number, colName: string): unknown {
...
function findRowByColumn(tableName: string, colName: string, value: unknown): number {
  var sheet = getSheetByName(tableName); if (!sheet || !sheet.content) return -1;
  var headers = sheet.content[0] || []; var colIdx = headers.indexOf(colName);
  if (colIdx === -1) return -1;
  for (var i = 1; i < sheet.content.length; i++) {
    const row = sheet.content[i];
    if (row && String(row[colIdx]) === String(value)) return i;
  }
  return -1;
}

// esbuild wraps this TypeScript source in an IIFE. Publish only the three
// historical helpers that the Shujuku host and existing worldbook scripts use.
window.getSheetByName = getSheetByName;
window.getCellByHeader = getCellByHeader;
window.findRowByColumn = findRowByColumn;

function safeJsonParse(str: string): AdapterRecord { try { return JSON.parse(str); } catch(e) { return {}; } }

window.getAllVariables = function(): AdapterVariables {
  var data: AdapterVariables = { stat_data: {} };
  var sd = data.stat_data;

  function sheet(tableName: string): AdapterSheet | null {
    try {
  ...
  function eachRow(tableName: string, callback: (row: AdapterRow, rowIndex: number, headers: AdapterRow) => void): void {
  ...
  function parseObject(value: unknown): AdapterRecord {
    try {
      if (!value) return {};
      if (typeof value === 'object') return value as AdapterRecord;
  ...
  function rankSortValue(value: unknown): number {
    ...
    var digitMap: Record<string, number> = { '零':0, '一':1, '二':2, '两':2, '三':3, '四':4, '五':5, '六':6, '七':7, '八':8, '九':9 };
    if (digitMap.hasOwnProperty(raw)) return digitMap[raw] ?? 999;
    var total = 0;
    var rest = raw;
    ...
    if (rest && digitMap.hasOwnProperty(rest)) total += digitMap[rest] ?? 0;
    return total > 0 ? total : 999;
  }

  function normalizeMessageHistory(value: unknown): AdapterRecord {
    var parsed = parseObject(value);
    var normalized: AdapterRecord = {};
    function addRecord(record: any, index: number): void {
  ...
  try {
    ...
    if (heroRow !== -1) {
      var h = function(c: string): unknown {
        try {
    ...
    sd.道侣 = {};
    eachRow('道侣表', function(r: AdapterRow) {
      if (!r[1]) return;
      sd.道侣[text(r[1])] = {
    ...
    sd.灵宠 = {};
    eachRow('灵宠表', function(r: AdapterRow) {
      if (!r[1]) return;
      sd.灵宠[text(r[1])] = {
    ...
    sd.人物 = {};
    eachRow('NPC表', function(r: AdapterRow) {
      if (!r[1]) return;
      sd.人物[text(r[1])] = {
    ...
    var lineKey = '$器灵台词';
    sd[lineKey] = [];
    eachRow('器灵台词表', function(r: AdapterRow) { if (r[1]) sd[lineKey].push(text(r[1])); });
  } catch(e) {
